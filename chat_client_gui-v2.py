...
class GuiChatClient:
    """A graphical, secure chat client using Tkinter."""
    def __init__(self, root):
        ...
        self.host = ""
        self.port = 5555  # Default server port
        self.username = ""
        self.password = ""
        ...
        # Queue for thread-safe communication between the network thread and GUI thread
    ...
    def show_login_dialog(self):
        ...
        tk.Label(dialog, text="Username (optional):").pack(pady=(10, 0))
    ...
    def attempt_connection(self, dialog):
        ...
        # Username is optional, but Server IP and Password are required.
        if not all([self.host, self.password]):
            messagebox.showerror("Error", "Server IP and Password are required.", parent=dialog)
            return
    ...
    def setup_chat_ui(self):
        ...
        # The title will be updated later if the username is defaulted
    ...
    def start_client(self):
        ...
        try:
            # Setup encryption using a fixed salt
            ...
            # If the username is empty, set it to the client's ephemeral port number.
            if not self.username.strip():
                # getsockname() returns (host, port) of the local socket
                client_port = self.client_socket.getsockname()[1]
                self.username = str(client_port)
                # Update the window title now that we have the final username
                self.root.title(f"Secure Chat - {self.username}")
    ...
    def send_message(self, event=None):
        """Encrypts and sends a message from the input entry to the server."""
    ...
    def on_closing(self):
        ...
        if self.client_socket:
            try:
                ...
                print(f"Error closing socket: {e}") # Optional: for debugging
        self.root.destroy()
