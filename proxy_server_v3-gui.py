...
class ProxyServer:
    ...
    def stop(self):
        ...
        # Use a try-except block in case the socket is already closed
        try:
            self.server_socket.close()
        except socket.error:
            pass # Socket was likely already closed
...
class ServerGUI:
    def __init__(self, root):
        ...
        self.generate_button = tk.Button(top_frame, text="Generate Code", command=self.generate_code, bg="#2196F3", fg="white")
    ...
    # --- ADDED: Method to generate the random code ---
    def generate_code(self):
        """Generates a random 3-letter, 3-number code and logs it."""
        letters = ''.join(random.choices(string.ascii_uppercase, k=3))
        numbers = ''.join(random.choices(string.digits, k=3))
        code = f"{letters}-{numbers}"
        self.log_message(f"[+] Generated Code: {code}")
    # -----------------------------------------------
